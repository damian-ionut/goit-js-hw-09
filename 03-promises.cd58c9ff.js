...
async function handleSubmit(event) {
    ...
    for(let i = 0; i < amount.value; i++)try {
        const result = await createPromise(i + 1, Number(delay.value) + Number(step.value) * i);
        (0, _notiflixDefault.default).Notify.success(`✅ Fulfilled promise ${result.position} in ${result.delay}ms`);
    } catch (error) {
        (0, _notiflixDefault.default).Notify.failure(`❌ Rejected promise ${error.position} in ${error.delay}ms`);
    } finally{
        if (i === Number(amount.value) - 1) formBtn.removeAttribute("disabled");
    }
}
